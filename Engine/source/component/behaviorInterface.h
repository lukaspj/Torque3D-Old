...
private:
   SimObjectPtr<BehaviorInstance> mOwner; ///< BehaviorObject will directly modify this value

public:
   /// Default constructor
   BehaviorInterface() : mOwner(NULL) {};
   ...
   BehaviorInstance *getOwner() { return mOwner; }
   const BehaviorInstance *getOwner() const { return mOwner; }
};

typedef VectorPtr<BehaviorInterface *> BehaviorInterfaceList;
typedef VectorPtr<BehaviorInterface *>::iterator BehaviorInterfaceListIterator;
...
/// a BehaviorObject to expose a property with custom processing code in an efficient
...
template<class T>
class BehaviorProperty : public BehaviorInterface
{
...
public:
   /// Default Constructor.
   BehaviorProperty() : mValuePtr( NULL )
   {
      mValuePtr = new T;
   }

   /// Copy constructor
   BehaviorProperty( const T &copy )
   {
      ComponentProperty();
...
private:
   ...
   {
      BehaviorInterface *iface;
      StringTableEntry type;
      StringTableEntry name;
      const BehaviorInstance *owner;
   };
...
public:
   ...
   virtual bool add( const char *type, const char *name, const BehaviorInstance *owner, BehaviorInterface *cinterface );

	virtual bool remove( const char *type, const char *name, const BehaviorInstance *owner);
   ...
   /// @param owner Limit results to components owned/not-owned by this BehaviorObject (see next param)
   /// @param notOwner If set to true, this will enumerate only interfaces NOT owned by 'owner'
   virtual U32 enumerate( BehaviorInterfaceList *list, const char *type = NULL, const char *name = NULL, const BehaviorInstance *owner = NULL, bool notOwner = false ) const;
};
